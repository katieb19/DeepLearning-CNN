...
def get_tiny_images(image_paths):
    ...
    #resize the original image to small resolution (16x16)
    resized = np.zeros((len(image_paths), 256))

    for image in range(0, len(image_paths)):
        newImage = imread(image_paths[image])
        if len(newImage.shape) == 3:
            newImage = rgb2grey(newImage)
        newImage = resize(newImage, (16, 16))
        newImage = np.reshape(newImage, (1, 256))
...
def build_vocabulary(image_paths, vocab_size):
    ...
    #pick a featrue length, 
    feature_length = 9
    # create array of number of images, number of columns the feature length
    finalArray = np.empty((0, (4*4*9)))

    #TODO: Implement this function!
    
    num_imgs = len(image_paths)

    # for each image paths get image --> convert to gray --> get histogram gradient (HOG)  
    for i in progressbar(range(num_imgs), "Loading ...", num_imgs):
        newImage = imread(image_paths[i])
        if len(newImage.shape) == 3:
            newImage = rgb2grey(newImage)
        HOG = hog(newImage, cells_per_block=(4, 4), pixels_per_cell= (8, 8), feature_vector= True)
        
        # reshape HOG to feature reshape (-1, picked)
        HOG = HOG.reshape(-1, (4*4*9))
        # add this feature to the features in an image (add to array)
        np.append(finalArray, HOG)

    #run k means on the array
    vocab_size = MiniBatchKMeans(vocab_size, max_iter = 100).fit(HOG)

    return vocab_size.cluster_centers_
    # return result of k means (cluster centers) sklearn.cluster.KMeans
    #$return np.array([])
...
def get_bags_of_words(image_paths):
    ...
    #pick a featrue length, 
    feature_length = 9
    # create array with rows equal to number of images, number of columns equivalent to vocab size
    ...
    for i in range(0, num_imgs):
        ...
        if len(newImage.shape) == 3:
            newImage = rgb2grey(newImage)
        featureHOG = hog(newImage, cells_per_block=(4, 4), pixels_per_cell= (8, 8), feature_vector= True)
        
        # reshape HOG to feature reshape (-1, picked) to get the feature vector
        featureHOG = featureHOG.reshape(-1, (4*4*9))

        #build histogram with the words
        distance = cdist(featureHOG, vocab, "euclidean")

        vocabsort = np.zeros((vocab.shape[0]))
        # find the closest vocab word
        for row in distance:
            sortedA = argsort(row)
            #add one to histogram at that bin
            vocabsort[sortedA[0]] += 1

        #optimization lin alg norm for feature HOG
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    classify = LinearSVC().fit(train_image_feats, train_labels).predict(test_image_feats)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    for i in range(0, distances.shape[0]):
        sortedFeature = np.argsort(distances[i])
        closest = sortedFeature[:k]
    # 2) Determine the labels of those k features
        temp_list = []
        for label in closest:
            temp_list.append(train_labels[label])
    # 3) Pick the most common label from the k
        common_label = mode(temp_list)[0]
